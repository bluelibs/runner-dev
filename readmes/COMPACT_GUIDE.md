...
- `typescript` is an optional peer (`>=5.0.0`, so npm installs next to any version): loading runner-dev never needs it, but `swapTask`, `eval` and `shell` need TypeScript 5 or 6 and return an install hint without it (or an incompatibility error with TypeScript 7)
