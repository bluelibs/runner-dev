...
- To reach it from Docker, a remote box or a LAN, set `dev.with({ host: "0.0.0.0" })` (or `resources.server.with({ host })`), plus `allowedHosts: ["devbox.lan"]` for access by DNS name; both entry points reject an entry with a scheme or port at `.with()`. There is no auth, and the server logs a startup warning when the bound address is not loopback and code execution is also enabled.
