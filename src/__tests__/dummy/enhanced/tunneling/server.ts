...
export const validatePaymentTask = r
  ...
  .run(async (input) => {
    console.log(`Validating ${input.paymentMethod} payment for ${input.amount} ${input.currency}`);
    ...
    if (!isValid) {
      return {
        ...
        riskScore,
        requires3DS,
        message: "Payment validation failed: Insufficient funds or invalid card details",
      };
    }
...
export const fetchExternalInventoryTask = r
  ...
  .run(async (input) => {
    console.log(`Fetching inventory from supplier ${input.supplierId} for ${input.productSkus.length} products`);
    ...
    const inventory = input.productSkus.map(sku => ({
      ...
      price: input.includePricing ? Math.floor(Math.random() * 100) + 10 : undefined,
    ...
    return {
      ...
      inventory,
      totalAvailable,
      nextRestock: Math.random() > 0.5 ? new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) : undefined,
    };
  })
  .build();
...
export const generateReportTask = r
  ...
  .run(async (input) => {
    console.log(`Generating ${input.reportType} report from ${input.dateRange.startDate} to ${input.dateRange.endDate}`);

    // Simulate heavy computation
    await new Promise(resolve => setTimeout(resolve, 5000)); // 5 second delay

    const recordCount = Math.floor(Math.random() * 10000) + 1000;
    const fileSize = input.format === "pdf" ? recordCount * 1024 : recordCount * 512;
    const pageCount = input.format === "pdf" ? Math.ceil(recordCount / 50) : undefined;

    return {
      reportId: `report_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      downloadUrl: `https://reports.example.com/download/report_${Date.now()}.${input.format}`,
      fileSize,
...
export const fraudDetectionTask = r
  ...
  .run(async (input) => {
    console.log(`Running fraud detection for transaction ${input.transactionId} amount ${input.amount}`);

    // Simulate fraud detection processing
    await new Promise(resolve => setTimeout(resolve, 1000)); // 1 second delay

    const baseRiskScore = Math.floor(Math.random() * 30);
    const amountRisk = input.amount > 1000 ? Math.min(20, input.amount / 100) : 0;
    const locationRisk = input.billingAddress.country !== input.shippingAddress?.country ? 15 : 0;
    const totalRiskScore = Math.min(100, baseRiskScore + amountRisk + locationRisk);
    ...
    return {
      ...
      factors,
      recommendation,
      sessionId: `fraud_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
    };
  })
  .build();
