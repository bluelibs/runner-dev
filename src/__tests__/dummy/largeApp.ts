...
// ====================
// RANDOM APP GENERATOR (CONFIGURABLE)
// ====================

/**
 * Create a seeded PRNG for deterministic generation.
...
function pickManyUnique<T>(rng: () => number, arr: T[], count: number): T[] {
  const result: T[] = [];
...
function maybeTags<T extends { with: (cfg?: any) => any }>(
  rng: () => number,
  pool: T[],
  maxPerItem: number
) {
  const count = randInt(rng, 0, Math.min(maxPerItem, pool.length));
  const picked = pickManyUnique(rng, pool, count);
  // Use tag.with to allow tagging with optional config where supported.
  return picked.map((t) => ((t as any).with ? (t as any).with({}) : t));
}

export type RandomAppConfig = {
  resources?: number; // default 100
  tasks?: number; // default 300
  events?: number; // default 100
  tags?: number; // default 10
  maxMiddlewarePerTask?: number; // default 5
  // Dependency ranges per task
  taskResourceDepsRange?: [min: number, max: number]; // default [1, 3]
  taskEventDepsRange?: [min: number, max: number]; // default [0, 2]
  seed?: number; // default Date.now()
};

/**
 * Programmatically generates a large app with random resources, tasks, events, middleware and tags.
 * All counts are configurable and generation is deterministic with a seed.
 */
export function createRandomApp(config: RandomAppConfig = {}) {
  const {
    resources: RES_COUNT = 100,
    tasks: TASK_COUNT = 300,
    events: EVT_COUNT = 100,
    tags: TAG_COUNT = 10,
    maxMiddlewarePerTask = 5,
    taskResourceDepsRange = [1, 3],
    taskEventDepsRange = [0, 2],
    seed = Date.now(),
  } = config;
    ...
    (_, i) =>
      taskMiddleware<{ level?: "debug" | "info" | "warn" | "error" }>({
        ...
        async run({ task, next }, _evt, _config) {
          ...
          const duration = Date.now() - start;
          if (duration > 1000) {
            console.warn(`[MW] ${task.definition.id} slow: ${duration}ms`);
          }
          return res;
        },
  ...
  // Helper factories for task deps
  function makeTaskDeps(
    resourceCountRange: [number, number],
    eventCountRange: [number, number]
  ) {
    const [minRes, maxRes] = resourceCountRange;
    const [minEvt, maxEvt] = eventCountRange;
    const resCount = randInt(rng, minRes, maxRes);
    const evtCount = randInt(rng, minEvt, maxEvt);

    const resPicked = pickManyUnique(rng, resourceDefs, resCount);
    const evtPicked = pickManyUnique(rng, eventDefs, evtCount);

    const deps: Record<string, any> = {};
    resPicked.forEach((r, idx) => {
      deps[`res${idx + 1}`] = r;
    });
    evtPicked.forEach((e, idx) => {
      deps[`emit${String(idx + 1).padStart(2, "0")}`] = e;
    });
    return deps;
  }
  ...
  const taskDefs = Array.from({ length: TASK_COUNT }, (_, i) => {
    ...
    const middlewares = (() => {
      const count = randInt(
        rng,
        0,
        Math.min(maxMiddlewarePerTask, middlewarePool.length)
      );
      // Pick count unique middlewares randomly
      ...
      async run(input, deps: Record<string, any>) {
        ...
        for (const key of Object.keys(deps)) {
          if (key.startsWith("emit")) {
            try {
              await deps[key]({ seq: i + 1, payload: { id, input } });
            } catch {
              /* intentionally empty */
            }
          }
  ...
  return resource({
    ...
    register: [
      // Tags first so they can be referenced
      ...tagDefs,
      // Middleware next
      ...middlewarePool,
      // Events, then resources, then tasks
      ...eventDefs,
      ...resourceDefs,
      ...taskDefs,
