export interface TreeNode {
  id: string;
  label: string;
  type: 'folder' | 'task' | 'resource' | 'event' | 'hook' | 'middleware' | 'tag';
  icon: string;
  children: TreeNode[];
  count?: number;
 ...
 */
export function getElementType(element: Element): 'task' | 'resource' | 'event' | 'hook' | 'middleware' | 'tag' {
  ...
  if (parts.length >= 2) {
    ...
    if (['middlewares', 'middleware'].includes(typeSegment)) return 'middleware';
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (!currentLevel.has(currentPath)) {
        ...
        if (i > 0) {
          ...
          if (parent && !parent.children.find(child => child.id === currentPath)) {
 ...
 */
export function toggleNodeExpansion(nodes: TreeNode[], nodeId: string, expanded?: boolean): TreeNode[] {
 ...
 */
export function expandPathToNode(nodes: TreeNode[], targetNodeId: string): TreeNode[] {
  const expandPath = (currentNodes: TreeNode[], path: string[] = []): TreeNode[] => {
    return currentNodes.map(node => {
      const newPath = [...path, node.id];
      
      // Check if this node or any of its children contain the target
      const containsTarget = node.id === targetNodeId || 
